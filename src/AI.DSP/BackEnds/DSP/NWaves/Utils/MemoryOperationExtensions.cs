...
namespace AI.BackEnds.DSP.NWaves.Utils
{
    ...
    public static class MemoryOperationExtensions
    {
        ...
        public static float[] FastCopyFragment(this float[] source, int size, int sourceOffset = 0, int destinationOffset = 0)
        {
            ...
            float[] destination = new float[totalSize];
            Buffer.BlockCopy(source, sourceOffset * _32Bits, destination, destinationOffset * _32Bits, size * _32Bits);
            return destination;
        }
        ...
        {
            float[] merged = new float[source1.Length + source2.Length];
        ...
        {
            float[] repeated = new float[source.Length * times];
        ...
        {
            float[] zeroPadded = new float[size];
        ...
        {
            int totalSize = size + destinationOffset;
            double[] destination = new double[totalSize];
            Buffer.BlockCopy(source, sourceOffset * _64Bits, destination, destinationOffset * _64Bits, size * _64Bits);
            return destination;
        }
        ...
        {
            double[] merged = new double[source1.Length + source2.Length];
        ...
        {
            double[] repeated = new double[source.Length * times];
        ...
        {
            double[] zeroPadded = new double[size];
